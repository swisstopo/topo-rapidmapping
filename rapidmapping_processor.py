...
def _assign_sequential_ms_offsets(
    ...
    for ts_key, group in groups.items():
        if len(group) == 1:
            # Eindeutiger Timestamp → kein Offset nötig
            result[group[0]] = ts_key
        else:
...
def convert_tif_to_jpg_with_exif(
    ...
    # ms-Offset ins JPEG-Dateinamen kodieren, damit process_individual_photos
    # den eindeutigen Timestamp aus dem Dateinamen ableiten kann.
    # "2025:07:10 08:00:28:01" → stem suffix "_080028_01"
    # "2025:07:10 08:00:28"    → stem suffix "_080028"    (kein Offset)
    if dt_str and len(dt_str) > 19:
        # hat ms-Offset: "YYYY:MM:DD HH:MM:SS:NN"
        time_part  = dt_str[11:19].replace(':', '')   # "080028"
        ms_part    = dt_str[20:]                       # "01"
    ...
    if needs_gdalwarp:
        # gdalwarp direkt TIF → JPEG: reprojeziiert auf nord-oben EPSG:4326
        # Das Georeferenzierungssystem des TIF ist bekannt → gdalwarp kann
        # die Pixel physisch korrekt neu anordnen.
        try:
...
def convert_tif_files_in_directory(
    ...
    # Statistik ausgeben
    unique_ts   = len(set(ts.split(':')[0] + ':' + ts.split(':')[1] + ':' + ts.split(':')[2]
                          for ts in ts_with_offset.values() if ts and not ts.startswith('__')))
    burst_count = sum(1 for ts in ts_with_offset.values()
                      if re.search(r':\d{2}$', ts) and int(ts[-2:]) > 1)
    logger.info(f"  Eindeutige Timestamps: ~{unique_ts}, Burst-Frames zum Überspringen: {burst_count}")

    # ------------------------------------------------------------------
    # PHASE 3: Auswahl und Sofort-Konvertierung (eine pro Timestamp-Gruppe)
    # ------------------------------------------------------------------
    jpg_files  = []
    skipped    = 0
    converted  = 0
    ...
    for tif in tif_files:
        ...
        # Burst-Frames :02, :03, ... überspringen
        # Pattern matcht nur das ms-Offset-Suffix nach vollständigem Datetime:
        # "2025:07:10 08:00:28:02" → match; "2025:07:10 08:00:28" → kein match
        ms_match = re.search(r'^\d{4}:\d{2}:\d{2} \d{2}:\d{2}:\d{2}:(\d{2})$', effective_ts)
        if ms_match and int(ms_match.group(1)) > 1:
            logger.info(f"  ⏭ Burst-Frame #{ms_match.group(1)} übersprungen: {tif.name}")
            skipped += 1
            continue

        # Repräsentative Datei → sofort konvertieren
    ...
    logger.info(
        f"  ✓ Konvertierung abgeschlossen: {converted} konvertiert, "
        f"{skipped} Burst-Frame(s) übersprungen"
    )
    return jpg_files
...
def parse_exif_timestamp(exif_timestamp: Optional[str], photo_name: str) -> str:
    ...
    if exif_timestamp:
        try:
            # Erweitertes Format mit ms-Offset: "YYYY:MM:DD HH:MM:SS:NN"
            ext_match = re.match(
                r'(\d{4}:\d{2}:\d{2} \d{2}:\d{2}:\d{2}):(\d{2})$',
                exif_timestamp
            )
            if ext_match:
                base_ts  = ext_match.group(1)
                ms_part  = ext_match.group(2)
                dt = datetime.strptime(base_ts, "%Y:%m:%d %H:%M:%S")
                return dt.strftime("%Y-%m-%dt%H%M%S").lower() + ms_part
...
def process_individual_photos(
    ...
    try:
        # ====================================================================
        # TIF → JPEG VORVERARBEITUNG (NEU v2.1 / v2.2)
        # ====================================================================
        ...
        for idx, jpg_file in enumerate(jpg_files, 1):
            ...
            try:
                ...
                # TIF-konvertierte Dateien haben den ms-Offset im Dateinamen:
                # "001_id111cL150156_080028_01.jpg" → Timestamp inkl. Offset
                # Der ms-Offset ist NICHT im EXIF (EXIF enthält nur HH:MM:SS).
                # Für normale JPEGs (kein Offset im Stem) → EXIF wie bisher.
                stem = jpg_file.stem
                stem_ts_match = re.search(
                    r'_(\d{6})_(\d{2})$',   # "_080028_01" am Ende des Stems
                    stem
                )
                if stem_ts_match and exif_timestamp:
                    # ms-Offset aus Dateinamen + Basis-Datum aus EXIF zusammensetzen
                    ms_offset = stem_ts_match.group(2)
                    try:
                        dt = datetime.strptime(exif_timestamp, "%Y:%m:%d %H:%M:%S")
                        timestamp = dt.strftime("%Y-%m-%dt%H%M%S").lower() + ms_offset
                        logger.info(f"     ✓ Timestamp mit ms-Offset: {timestamp}")
