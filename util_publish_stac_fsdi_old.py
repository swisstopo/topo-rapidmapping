...
def item_create_json_payload(id, coordinates, dt_iso8601, title, geocat_id, current, stac_hostname, asset):
    ...
    thumbnail_url = f"{domain}ch.swisstopo.{product}/{id}/thumbnail.png"
...
def publish_to_stac(asset, item_name, collection, geocat_id,
    ...
    environment = "INT" if ".int." in stac_hostname else "PROD"
    creds=load_stac_credentials(credentials_path=None, environment=environment)
    username=creds[0]
    password =creds[1]
